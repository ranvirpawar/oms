...
class HandoverConnectorView extends StatelessWidget {
  ...
  const HandoverConnectorView({
    super.key,
    this.isHandOverToRunnerBoy = false,
  });

  @override
  Widget build(BuildContext context) {
    if (kDebugMode) {
      print('❤️❤️❤️❤️❤️HandoverConnectorView: isHandOverToRunnerBoy: $isHandOverToRunnerBoy');
    }
    Get.put(HandoverConnectorController(isHandOverToRunnerBoy: isHandOverToRunnerBoy));

    final controller = Get.find<HandoverConnectorController>();

    return Scaffold(
      backgroundColor: const Color(0xFFF5F7FA),
      appBar: CustomAppBar(
        title: isHandOverToRunnerBoy ? 'Assign to Runner Boy' : 'Handover to Connector',
      ),
      body: Obx(() {
        if (controller.handoverState.value == HandoverState.processing && controller.handoverProgress.value > 0) {
          return _buildProcessingView(controller);
        }
        if (controller.handoverState.value == HandoverState.success) {
      ...
      bottomNavigationBar: Obx(() {
        if (controller.handoverState.value == HandoverState.success) {
          return _buildSuccessBottomBar(controller);
        }
  ...
  Widget _buildMainView(HandoverConnectorController controller) {
    return SingleChildScrollView(
      child: Column(
        children: [
          // Bag Type: Only show dropdown for Connector flow
       _buildBagTypeSection(controller),


          _buildConnectorSection(controller),
          Obx(() => controller.isScanning.value
              ? _buildScannerSection(controller)
              : _buildManualEntrySection(controller)),
          const SizedBox(height: 16),
          _buildScannedBagsList(controller),
  ...
  Widget _buildEmptyBagLabel() {
    return Container(
      ...
      child: Row(
        children: [
          SvgPicture.asset(AppAssets.medicalUnitIcon, height: 22,
              colorFilter: const ColorFilter.mode(AppColors.primary, BlendMode.srcIn)),
          const SizedBox(width: 12),
          const Text('Bag Type', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
          const Spacer(),
          Container(
            ...
            child: const Text('Empty Bag', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.primary)),
          ),
        ],
      ),
    );
  ...
  Widget _buildBagTypeSection(HandoverConnectorController controller) {
    return Container(
      margin: const EdgeInsets.all(16),
  ...
  // -----------------------------------------------------------------
  Widget _buildConnectorSection(HandoverConnectorController controller) {
    final title = controller.isHandOverToRunnerBoy ? 'Assign to Runner Boy' : 'Assign to Connector';
    final icon = controller.isHandOverToRunnerBoy
        ? (AppAssets.connectorIcon ?? AppAssets.connectorIcon)
  ...
  // -----------------------------------------------------------------
  Widget _buildScannerSection(HandoverConnectorController controller) {
    return Container(
  ...
  // 4. Manual entry (same)
  // -----------------------------------------------------------------
  Widget _buildManualEntrySection(HandoverConnectorController controller) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 16),
  ...
  // -----------------------------------------------------------------
  Widget _buildScannedBagsList(HandoverConnectorController controller) {
    return Obx(() {
          ...
          children: [
            ...
            ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return _buildBagCard(bag, controller);
              },
            ),
          ],
        ),
  ...
  Widget _buildBagCard(BagTransaction bag, HandoverConnectorController controller) {
  ...
  // -----------------------------------------------------------------
  Widget _buildBottomBar(HandoverConnectorController controller) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: Colors.white,
  ...
  // -----------------------------------------------------------------
  Widget _buildSuccessView(HandoverConnectorController controller) {
    return Container(
      color: Colors.white,
      child: Center(
        child: Column(
  ...
  // -----------------------------------------------------------------
  Widget _buildSuccessBottomBar(HandoverConnectorController controller) {
    return Padding(
  ...
  // -----------------------------------------------------------------
  Widget _buildProcessingView(HandoverConnectorController controller) {
    return Container(
      color: Colors.white,
      child: Center(
        child: Column(
