...
/// the reschedule flow. The backend keys slots by [slotId] — the reschedule
/// insert API expects that id, so the UI must always offer slots from this
/// list instead of inventing free-form times.
class AvailableSlot {
  final int slotId;
  final DateTime? date;
  final String inTime;
...
/// A reschedule reason returned by the `GetRescheduleReasone` endpoint.
///
/// Selecting one is mandatory before confirming a reschedule — its [id] is
/// sent to the appointment-reschedule API as `RescheduleReasoneID`. Parsing
/// is lenient across the id/text key spellings the backend has historically
/// used (the endpoint name itself carries the legacy `Reasone` spelling).
class RescheduleReason {
  ...
  factory RescheduleReason.fromJson(Map<String, dynamic> json) {
    return RescheduleReason(
      id: _toInt(json['RescheduleReasoneID'] ??
          json['RescheduleReasonID'] ??
          json['RID']),
      reason: (json['RescheduleReasone'] ??
...
/// Mirrors the OrderStatus master table (OrderStatusID -> OrderStatusCode).
/// The numeric ID from the API (`OrderStatusID`) is now the single source
/// of truth for status — see [PatientStatusX.fromOrderStatusId].
///
///  1  NAS  Not Assigned
///  2  ASN  Assigned
///  3  OAC  Order Accepted
///  4  OTW  On The Way
///  5  ARR  Arrived
///  6  COL  Sample Collected
///  7  SPU  Sample Picked Up
///  8  SAL  Sample Accepted in Lab
///  9  SPI  Sample Processing In-Progress
/// 10  RAP  Report Available
/// 11  APR  Appointment Reschedule
/// 12  SRJ  Sample Rejected
/// 13  RAL  Received At LIS
enum PatientStatus {
  notAssigned,
  assigned,
  pending,
  ...
  /// to the patient. (OrderStatusID 4 / OTW)
  inRoute,
  inProgress,
  ...
  /// OrderStatusID 9 / SPI
  sampleProcessingInProgress,

  /// OrderStatusID 10 / RAP
  reportAvailable,

  /// OrderStatusID 12 / SRJ — distinct from the assignment-level
  /// [rejected] below; this means the *sample* was rejected after
  /// collection, not that the phlebotomist declined the assignment.
  sampleRejected,
...
extension PatientStatusX on PatientStatus {
  ...
  bool get isLisSyncFailed => this == PatientStatus.sampleCollected;
  ...
  bool get isTerminal =>
      this == PatientStatus.completed ||
          ...
          this == PatientStatus.sampleRejected ||
          this == PatientStatus.unableToCollect ||
          this == PatientStatus.reportAvailable;

  /// Single source of truth for status: maps the API's `OrderStatusID`
  /// (see OrderStatus master table) directly to a [PatientStatus].
  /// Replaces the old logic that derived status from the `Status` /
  /// `OrderStatus` strings and the `IsRoute` flag.
  static PatientStatus fromOrderStatusId(int? id) {
    switch (id) {
      case 1: // NAS - Not Assigned
        return PatientStatus.notAssigned;
      case 2: // ASN - Assigned
        return PatientStatus.assigned;
      case 3: // OAC - Order Accepted
        return PatientStatus.accepted;
      case 4: // OTW - On The Way
        return PatientStatus.inRoute;
      case 5: // ARR - Arrived
        return PatientStatus.arrived;
      case 6: // COL - Sample Collected
        return PatientStatus.sampleCollected;
      case 7: // SPU - Sample Picked Up
        return PatientStatus.samplePickedUp;
      case 8: // SAL - Sample Accepted in Lab
        return PatientStatus.sampleAcceptedInLab;
      case 9: // SPI - Sample Processing In-Progress
        return PatientStatus.sampleProcessingInProgress;
      case 10: // RAP - Report Available
        return PatientStatus.reportAvailable;
      case 11: // APR - Appointment Reschedule
        return PatientStatus.rescheduled;
      case 12: // SRJ - Sample Rejected
...
class AssignedPatient {
  ...
  /// Raw OrderStatusID from the API, kept alongside the parsed [status]
  /// enum in case callers need the exact numeric code (e.g. to re-send
  ...
  factory AssignedPatient.fromJson(Map<String, dynamic> json) {
    ...
    final orderStatusId = (json['OrderStatusID'] as num?)?.toInt();
    ...
    return AssignedPatient(
      ...
      // Status now comes directly from the OrderStatus master table via
      // OrderStatusID — no more Status/OrderStatus string parsing and no
      // more IsRoute override.
      status: PatientStatusX.fromOrderStatusId(orderStatusId),
