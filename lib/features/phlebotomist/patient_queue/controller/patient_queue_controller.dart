...
class PatientQueueController extends GetxController {
  ...
  bool _matchesDateFilter(AssignedPatient p) {
    ...
    switch (activeDateFilter.value) {
      case QueueDateFilter.today:
        ...
        return !slot.isBefore(today) && slot.isBefore(tomorrow);
      case QueueDateFilter.thisWeek:
        if (slot == null) return true;
        return !slot.isBefore(today) && slot.isBefore(weekEnd);
  ...
  /// [_visiblePatients] narrowed to the active visit-type tab + date
  /// horizon — the shared context that both the status chips and the
  /// final patient list are built from.
  ...
  /// Clinic-tab badge count — respects the active date horizon so it stays
  /// meaningful next to the tab (e.g. "Clinic 4" = 4 clinic patients today).
  int get clinicVisitCount => _visiblePatients
      .where((p) => p.visitType == VisitType.clinic)
      .where(_matchesDateFilter)
      .length;

  int get homeVisitCount => _visiblePatients
      .where((p) => p.visitType == VisitType.home)
      .where(_matchesDateFilter)
      .length;
  ...
  // within the current visit-type tab + date horizon (not the whole
