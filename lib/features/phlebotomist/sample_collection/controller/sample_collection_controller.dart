...
class SampleCollectionController extends GetxController with HasBagContext {
  ...
  // ---- Reschedule (the "bail out of collection" app-bar action) ---------
  ...
  SampleCollectionPayload _buildPayload() {
    ...
    return SampleCollectionPayload(
      ...
      orderStatusCode: incompleteTests.isEmpty
          ? 'COLLECTED'
          : 'PARTIALLY_COLLECTED',
      // The open bag's real identifiers, resolved fresh from the shared bag
  ...
  /// Submits the collection. Returns the outcome instead of a bare bool so
  /// the caller can distinguish a full success from "saved, but LIS sync
  /// failed" — both still route to the success screen. Null means nothing
  /// was saved.
  ...
  /// Call this from the submit button. Routes to the success screen on a
  /// full success or a partial-LIS-failure "soft" success; does nothing on
  /// a hard failure (the snackbar above already told the user what went
  /// wrong, so they stay on the form).
  ...
  /// Retries the push to Disha from the success screen. Also doubles as a
  /// manual "check status" action, since the endpoint's response tells you
  /// the current sync state either way.
  ...
  // ---------------------------------------------------------------------
  // Reschedule (app-bar "bail out" action)
  // ---------------------------------------------------------------------
